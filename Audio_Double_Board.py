...
# your_path = '/rds/general/user/zx719/home/Audio_Kaggle'
your_path = '/home/zx719/Documents/MresMLBD/MResProject/Audio_Kaggle'
Np = 3 #number of signal types
layer1_Np = 20 #number of probes on board 1
basedir = f'Double_Board/V4_HN{layer1_Np}_'+f'_{Np}sig'
...
timesteps = 1300  # number of timesteps for wave propagation (nx*5 is the defult wave propergation time)
...
for i in range(layer1_Np):
  # src2[i] = spintorch.WaveLineSource(10, int(10+(ny-20)/layer1_Np*i), 10, int(10+(ny-20)/layer1_Np*(i+1)), dim=2) #no signal at the sides
  # print(f'board 2 probe {i} position: ',10,  int(10+(ny-20)/layer1_Np*i), 10, int(10+(ny-20)/layer1_Np*(i+1)))
  src2[i] = spintorch.WaveLineSource(10, int((ny-1)/layer1_Np*i), 10, int((ny-1)/layer1_Np*(i+1)), dim=2) #send signals at the sides
  print(f'board 2 probe {i} position: ',10, int(ny/layer1_Np*i), 10, int(ny/layer1_Np*(i+1)))
...
# Fetching the device that will be used throughout this notebook        # if final_board == True:
...
for epoch in range(epoch_init+1, epoch_max):
  ...
  '''Plot spin-wave propagation every 25 epochs '''
  if epoch % 25 == 24 or epoch == 0:
    # plot output bars
...
for BATCH, data in enumerate(test_loader, 0):
    ...
    all_output = {'model_out_list':model_out_list,'OUTPUTS': OUTPUTS_test, 'loss': loss,'Normal_Prediction': predicted,'test_accuracy':test_accuracy, 'kcpred':kcpred,'kgpred':kgpred, 'knn_accuracy':knn_accuracy}
